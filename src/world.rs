...
impl World {
    ...
    pub fn render(&mut self, args: RenderArgs) {
        ...
        // renderuse std::collections::BinaryHeap;
        // use std::ops::Mul;
        // use std::thread;
        // use crate::BVH::BVH;
        // 
        ...
        //             let triangle_object_clone = triangle_object_s.clone();
        //             let tx = tx.clone();
        //             let target = move || {
        //                 for y in (i..height).step_by(args.threads as usize).tqdm() {
        //                     for x in 0..width {
        //                         let ray = world.camera.get_ray(x, y);
        //                         let mut buffered = world.camera.buffer[(x as usize, y as usize)].clone();
        //                         let color = world.coloring(&ray, &mut buffered, &triangle_object_clone, Status::air(0.0), args_clone.ttl);
        //                         tx.send((x, y, color)).unwrap();
        //                         // self.camera.picture[(x as usize, y as usize)] = color;
        //                     }
        //                 }
        ...
        //         //         let color = self.coloring(&ray, &mut buffered, &triangle_object_s, Status::air(0.0), args.ttl);
        //         //         self.camera.picture[(x as usize, y as usize)] = color;
        //         //     }
        //         // }
        // 
